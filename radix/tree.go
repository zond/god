...
func newNodeIterator(f TreeIterator) nodeIterator {
	return func(key []byte, byteValue []byte, treeValue *Tree, use int, version int64) (cont bool) {
		if use&ByteValue != 0 {
			return f(key, byteValue, version)
		}
		return true
	}
}
...
func newNodeIndexIterator(f TreeIndexIterator) nodeIndexIterator {
	return func(key []byte, byteValue []byte, treeValue *Tree, use int, version int64, index int) (cont bool) {
		if use&ByteValue != 0 {
			return f(key, byteValue, version, index)
		}
		return true
	}
}
...
func (self *Tree) Get(key []byte) (byteValue []byte, version int64, existed bool) {
	self.lock.RLock()
	defer self.lock.RUnlock()
	byteValue, _, version, ex := self.root.get(Rip(key))
	existed = ex&ByteValue != 0
	return
}
...
func (self *Tree) GetVersion(key []Nibble) (byteValue []byte, version int64, existed bool) {
	self.lock.RLock()
	defer self.lock.RUnlock()
	byteValue, _, version, ex := self.root.get(key)
	existed = ex&ByteValue != 0
	return
}
...
func (self *Tree) PutVersion(key []Nibble, byteValue []byte, expected, version int64) bool {
	self.lock.Lock()
	defer self.lock.Unlock()
	return self.putVersion(key, byteValue, nil, ByteValue, expected, version)
}
...
func (self *Tree) SubPutVersion(key, subKey []Nibble, byteValue []byte, expected, subExpected, subVersion int64) bool {
	self.lock.Lock()
	defer self.lock.Unlock()
	if _, subTree, subTreeVersion, ex := self.root.get(key); ex&TreeValue == 0 || (ex&TreeValue != 0 && subTree != nil && subTreeVersion == expected) {
		if subTree == nil {
			subTree = newTreeWith(subKey, byteValue, subVersion)
		} else {
			subTree.PutVersion(subKey, byteValue, subExpected, subVersion)
		}
